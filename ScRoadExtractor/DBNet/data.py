...
def default_loader111_no_augment(id, sat_dir, lab_dir, hed_dir):
    img = cv2.imread(os.path.join(sat_dir, '{}_sat.png').format(id))
    mask = cv2.imread(os.path.join(lab_dir+'{}_mask.png').format(id), cv2.IMREAD_GRAYSCALE)
    hed = cv2.imread(os.path.join(hed_dir+'{}_hed.png').format(id), cv2.IMREAD_GRAYSCALE)
    mask = np.expand_dims(mask, axis=2)
...
def default_loader111_greedy(id, sat_dir, lab_dir, hed_dir):
    img = cv2.imread(os.path.join(sat_dir, '{}_sat.png').format(id))
    mask = cv2.imread(os.path.join(lab_dir+'{}_mask.png').format(id), cv2.IMREAD_GRAYSCALE)
    hed = cv2.imread(os.path.join(hed_dir+'{}_hed.png').format(id), cv2.IMREAD_GRAYSCALE)
    return img, mask, hed

def transform(img, mask, hed):
  img = randomHueSaturationValue(img,
                                        hue_shift_limit=(-30, 30),
                                        sat_shift_limit=(-5, 5),
                                        val_shift_limit=(-15, 15))
...
class ImageFolder(data.Dataset):
    ...
    # def __init__(self, trainlist, sat_dir, lab_dir):
        ...
        self.loader111 = default_loader111_greedy
        self.sat_dir = sat_dir
        self.lab_dir = lab_dir
        self.hed_dir = hed_dir
        self.dataset = []
        try:
          print("Attempting to load pickled dataset")
          with open(r"dataset_0-4999.pkl", "rb") as pfile:
            self.dataset = pickle.load(pfile)
          with open(r"dataset_5000-10000.pkl", "rb") as pfile:
            self.dataset += pickle.load(pfile)
        except:
          self.dataset = []
          print("Loading pickled dataset FAILED, generating new dataset")
          for id in tqdm(self.ids[:5000]):
            self.dataset += [(self.loader111(id, self.sat_dir, self.lab_dir, self.hed_dir))]
          with open("dataset_0-4999.pkl", 'wb') as pfile:
           pickle.dump(self.dataset, pfile, protocol=pickle.HIGHEST_PROTOCOL)
          self.dataset = []
          gc.collect()
          for id in tqdm(self.ids[5000:]):
            self.dataset += [(self.loader111(id, self.sat_dir, self.lab_dir, self.hed_dir))]
          with open("dataset_5000-10000.pkl", 'wb') as pfile:
           pickle.dump(self.dataset, pfile, protocol=pickle.HIGHEST_PROTOCOL)
          raise Exception("New dataset created, please restart kernel")
          
          
    def __getitem__(self, index):
        # img, mask = self.loader(id, self.sat_dir, self.lab_dir)
        try:
          img, mask, hed = self.dataset[index]
          img, mask, hed = transform(img, mask, hed)
        except:
          img, mask, hed = self.dataset[0]
          img, mask, hed = transform(img, mask, hed)
